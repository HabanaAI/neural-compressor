...
if re.search("llama", args.model.lower()):
    ...
    user_model = LlamaForCausalLM.from_pretrained(
        ...
        # token='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'
    )
    # tokenizer = LlamaTokenizer.from_pretrained(args.model, token='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')
...
if device != 'cpu':
    try:
        ...
        raise 'device should be int if try to use cuda, for example 0'
...
if args.quantize:
    ...
    elif re.search("llama", user_model.config.model_type):
        op_type_dict = {".*": {"activation": {"algorithm": "kl"}}, }
    else:
        op_type_dict = {}
    ...
    #
    #     print('start min max tuning ...')
    #     tuner = MinMaxTuner(user_model, calib_dataloader, sq_minmax_info=model_sq_max_info, device=device)
    #     st = time.time()
    #     best_min_maxes = tuner.tune(
    #         lr=0.0025,
    #         min_max_lr=0.0025,
    ...
    #         search_iters=400
    #     )
    #     print(f'tuning minmax running time: {time.time() - st}')

    # if args.fake_quant:
    #     if not args.sq:
    #         sq_model = user_model
    #     # wrap the model for fake quantization
    #     from quantization.fake_quant import wrap_model
    #
    #     wrap_model(sq_model, model_sq_max_info, best_min_maxes)
